...
const ANIMATION_NAME: Record<FloatingShape["type"], string> = {
  heart: "heroFloatHeart",
  sparkle: "heroFloatSparkle",
  star: "heroFloatGeneric",
  circle: "heroFloatGeneric",
  hand: "heroFloatGeneric",
  balloon: "heroFloatGeneric",
};

function FloatingElement({ shape }: { shape: FloatingShape }) {
  const Renderer = shapeRenderers[shape.type];
  const animName = ANIMATION_NAME[shape.type];
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: `${animName} ${shape.duration}s ease-in-out ${shape.delay}s infinite`,
      }}
    >
      <div style={{ transform: `rotate(${shape.rotate ?? 0}deg)` }}>
...
function RainbowArc() {
  ...
  return (
    <div className="absolute -bottom-32 -end-20 w-[500px] h-[500px] pointer-events-none">
      {colors.map((color, i) => (
        <div
          key={i}
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-[0.06]" aria-hidden>
        <circle cx="8%" cy="15%" r="2" fill="white"
          style={{ animation: "heroConstellationPulse 3s ease-in-out infinite" }}
        />
        <circle cx="45%" cy="70%" r="2" fill="white"
          style={{ animation: "heroConstellationPulse 3s ease-in-out 1s infinite" }}
        />
        <circle cx="72%" cy="22%" r="2" fill="white"
          style={{ animation: "heroConstellationPulse 3s ease-in-out 2s infinite" }}
        />
        <line x1="8%" y1="15%" x2="45%" y2="70%" stroke="white" strokeWidth="0.5" strokeDasharray="4 4" opacity="0.5" />
        <line x1="45%" y1="70%" x2="72%" y2="22%" stroke="white" strokeWidth="0.5" strokeDasharray="4 4" opacity="0.5" />
      </svg>
    </div>
  );
}
